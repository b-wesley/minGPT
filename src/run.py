# Most of this is based on solutions people posted online because a lot of this is above my paygrade
# i didn't copy/paste though, tried to actually digest and figure out what these people were doing
...
def main():
    ...
    if args.function == 'pretrain':
        ...
        # make the config and trainer, not 
        config = TrainerConfig(max_epoch=650, batch_size=128, learning_rate=6e-3, lr_decay=True, warmup_tokens=512*20, 
                               final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4)
        
        trainer = Trainer(model, pretrain_dataset, None, config)
        trainer.train()
        ...
        # load the args into model if it was given
        if args.reading_params_path != None:
            model.load_state_dict(torch.load(args.reaading_params_path))
            
        
        # make a trainer config object with the desired hyperparams
        config = TrainerConfig(max_epochs=75, batch_size=256, 
                               learning_rate=6e-4, lr_decay=True, warmup_tokens=512*20, 
                               final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4)    
        
        
        # load the corpus data into a dataset
        txt = open(args.finetune_corpus_path, 'r').read()
        train_data  = dataset.NameDataset(pretrain_dataset, txt)
        
        # make trainer object and train the model, then save it
        trainer = Trainer(model, train_data, None, config)
        trainer.train()
